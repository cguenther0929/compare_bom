...
def clean_value(textin):
    ...
    if isinstance(textin, bytes):
        value = textin.decode("utf-8")

    return str(textin).strip()
...
if __name__ == '__main__':
        ...
        if(files[i].upper().endswith(".XLSX")):
            ...
            for sh in range (num_sheets):
                ...
                for r in range (1,num_rows + 1):					# Find the header locations. Excel starts counting at one
                    ...
                    print (f"Search header before starting:  {search_header}")
                    ...
                    for c in range (1,num_cols + 1):				# Excel starts counting at 1
                        ...
                        temptext = clean_value(current_sheet.cell(row=r, column=c).value)
                        ...
                        if((len(search_header) == 1) and ("REF" in search_header) and (r) and (c == (num_cols+1))):
                            REF_col = 30
                            print (f"Current search header:  {search_header}")
                            search_header.remove("REF")
                            print("There is no reference field in this BOM. All other header fields found.")
                            logging.info("There is no reference field in this BOM. All other header fields found.")
                        
                        elif(len(search_header) == 0):
                            break;          #Everything has been found, so break out
                        ######
                    ...
                    if( (len(search_header) == 0) ):		# Found all header fields
                        ...
                        temp_qpn = clean_value(current_sheet.cell(row=data_start, column=QPN_col).value)
                        temp_des = clean_value(current_sheet.cell(row=data_start, column=DES_col).value)
                        temp_ref = clean_value(current_sheet.cell(row=data_start, column=REF_col).value)
                        temp_qty = clean_value(current_sheet.cell(row=data_start, column=QTY_col).value)
                        print(f"Sample data in start row: {temp_qpn} | {temp_des} | {temp_ref} | {temp_qty}")
                        
                        break

                    elif((r == 10) and (len(search_header) > 0) and sh < num_sheets):
                        sheet_valid = False
                        print (f"* File: {str(files[i])} contains invalid sheet: {str(ws[sh])} since these heading weren't located: {search_header} ")
                        break
                    
                if(sheet_valid):
                    print (f"QPN column found to be: {QPN_col}")		
                    print (f"MFGPN column found to be: {MFGPN_col}")		
                    ...
                    for r in range (data_start,num_rows + 1):
                        ...
                        temp_qpn = clean_value(current_sheet.cell(row=r, column=QPN_col).value)
                        temp_des = clean_value(current_sheet.cell(row=r, column=DES_col).value)
                        temp_ref = clean_value(current_sheet.cell(row=r, column=REF_col).value)
                        temp_qty = clean_value(current_sheet.cell(row=r, column=QTY_col).value)

                        if len(temp_qpn) <= 1 and len(temp_des) <= 1 and len(temp_ref) <= 1 and len(temp_qty) <= 1:
                            blank_row_count += 1
                            print(f"Blank row detected at row ({r})")
                            ...
                            blank_row_count = 0             # Reset the blank row counter
                            
                            temp_qpn    = clean_value(current_sheet.cell(row=r, column=QPN_col).value)
                            temp_des    = clean_value(current_sheet.cell(row=r, column=DES_col).value)
                            temp_ref    = clean_value(current_sheet.cell(row=r, column=REF_col).value)
                            temp_qty    = clean_value(current_sheet.cell(row=r, column=QTY_col).value)
                            temp_mfgpn  = clean_value(current_sheet.cell(row=r, column=MFGPN_col).value)
                            print(f"Sample data in current row: {temp_qpn} | {temp_mfgpn} | {temp_des} | {temp_ref} | {temp_qty}")
    ...
    NewSheet.column_dimensions['O'].width = 15			# QTY Discrepancy 			
    ...
    comparison_bom_col_offsets = {"T2_MFGPN":2,"T1_MFGPN":1,"T2_QPN":5,"T1_QPN":4,"T2_DES":8,"T1_DES":7,"T2_REF":11,"T1_REF":10,"T2_QTY":14,"T1_QTY":13,"QTY_DIFF":16}  #Excel row offsets are one-base!
    ...
    for key in dict_type1_bom:
        if (key not in dict_type2_bom):
            ...
            NewSheet.cell(row=current_row_counter,column=comparison_bom_col_offsets["T1_QPN"]).value = dict_type1_bom[key][0]
